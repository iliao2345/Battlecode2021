...
public class Muckraker {
	...
	public static void act() throws GameActionException {
		...
		if (best_gains>Double.MIN_VALUE || best_gains<-Double.MIN_VALUE) {
			...
			Role.attach_to_relay_chain();
			if (Info.n_enemy_slanderers>0) {RelayChain.lock_target(Info.closest_robot(Info.enemy, RobotType.SLANDERER).location);}
			else if (!Info.everything_buried && Info.n_buriers>0) {RelayChain.lock_target(Info.weak_burier.location);}
