...
public class Flag {
	...
	public static void set_default_patrol() throws GameActionException {
		bits = new int[] {1, 3, 3, 3, 1, 3, 1, LOG_MAX_APPROX_DIST, 1};
		values = new int[] {(Phase.is_membrane)?1:0,
							Math.max(0, Membrane.layer_num),
							Membrane.push_signal,
							0,
							(Info.reabsorb)?1:0,
							Info.muckraker_warning_level,
							(Info.type==RobotType.SLANDERER && Info.need_protection)?1:0,
	...
	public static void set_ec() throws GameActionException {
		bits = new int[] {22, 1, 1};
		values = new int[] {0,
							(ECInfo.n_enemies_adjacent>0)?1:0,
							(ECInfo.form_membrane)?1:0
							};
	}
