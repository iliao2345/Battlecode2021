	...
	public static void update() throws GameActionException {
		...
		for (Direction dir:Math2.UNIT_DIRECTIONS) {
			...
			if (robot!=null) {
				...
				if (robot.team==Info.friendly && (robot.type==RobotType.ENLIGHTENMENT_CENTER && flag%2==1 ||
						(robot.type==RobotType.POLITICIAN || robot.type==RobotType.SLANDERER) && (flag>>9)%2==1) && (i==1||j==1)) {  // (flag>>9)%2 is the slanderer tag
					layer_nums[i][j] = -3;
					layer_num = -2;
					is_membrane[i][j] = true;
					friendly[i][j] = true;
			...
			for (RobotInfo robot2:Info.friendly_ecs) {
				if (rc.getFlag(robot2.ID)%2!=1) {continue;}
				if (robot2.location.isWithinDistanceSquared(Info.loc.add(dir), 10)) {
					is_membrane[i][j] = true;
					friendly[i][j] = true;
					if (robot2.location.isWithinDistanceSquared(Info.loc.add(dir), 4)) {
			...
			for (RobotInfo robot2:Info.friendly_slanderers) {
				if (Info.friendly_slanderers.length>8) {break;}  // Try to save bytecode here
				if ((rc.getFlag(robot2.ID)>>9)%2==0) {continue;}
		...
		for (Direction dir:Math2.UNIT_DIRECTIONS) {
			if (is_membrane[dir.dx+1][dir.dy+1] && layer_nums[dir.dx+1][dir.dy+1]<=layer_num-1 && layer_nums[dir.dx+1][dir.dy+1]<4) {
...
//		if ((layer_num+4)%4==3) {rc.setIndicatorDot(Info.loc, 0, 0, 255);}
	...
	public static void heal() throws GameActionException {
		if (!touching_membrane || layer_num>=4) {  // evaporate if no membrane or thick membrane
			Phase.evaporate();
			return;
		}
		if (Info.type==RobotType.POLITICIAN && touching_enemy) {  // kill enemies if they get inside, especially if they get very close to the inside
		...
		if (Info.friendly_slanderers.length>8) {  // try to get away from the center if stuck inside membrane contents
			Pathing.target(Info.spawn_location, -1);
			return;
		}
		if (layer_num<0 && passable_exterior_dir!=null) {  // leave some space between the membrane and the membrane contents
			Action.move(passable_exterior_dir);
			return;
		}
		Direction interior_dir = null;  // try to fill gaps in the interior
		for (Direction dir:Math2.UNIT_DIRECTIONS) {
			if (interior[dir.dx+1][dir.dy+1]) {
		...
		if (closest_enemy_muckraker!=null) {  // thicken where enemies are nearby
...
//		rc.setIndicatorLine(Info.loc, Info.loc.add(right), 0, 0, 255);
	...
	// Try to maintain an equilibrium vapor pressure of the exterior gas
	public static void outgas() throws GameActionException {
		if (!touching_membrane || layer_num>=4) {  // evaporate if no membrane or thick membrane
			Phase.evaporate();
			return;
		}
		for (Direction dir:Math2.UNIT_DIRECTIONS) {  // don't evaporate if touching any exterior layer
			if (exterior[dir.dx+1][dir.dy+1] && is_membrane[dir.dx+1][dir.dy+1]) {return;}
		}
		if (touching_enemy && max_enemy_layer_num>layer_num) {return;}  // don't move if an enemy further away can immediately take your spot
		double evaporation_probability = 0;  // 0.5 makes the equilibrium go to 0 vapor pressure
		if (Info.type==RobotType.POLITICIAN) {  // politicians try to stay in the wall and absorb enemy damage to not waste the carried influence through empowerment
			switch (layer_num) {
			case 0: {evaporation_probability = 0.05; break;}
			case 1: {evaporation_probability = 0.05; break;}
			case 2: {evaporation_probability = 0.05; break;}
			case 3: {evaporation_probability = 0.2; break;}
			case 4: {evaporation_probability = 1; break;}
			case 5: {evaporation_probability = 1; break;}
			case 6: {evaporation_probability = 1; break;}
			case 7: {evaporation_probability = 1; break;}
			default: {evaporation_probability = 0; break;}
			}
		}
		if (Info.type==RobotType.MUCKRAKER) {  // muckrakers try to maintain about a 5:2 ratio of muckrakers to politicians in the gas
			switch (layer_num) {
			case 0: {evaporation_probability = 0.1; break;}
			case 1: {evaporation_probability = 0.1; break;}
			case 2: {evaporation_probability = 0.1; break;}
			case 3: {evaporation_probability = 0.4; break;}
			case 4: {evaporation_probability = 1; break;}
			case 5: {evaporation_probability = 1; break;}
			case 6: {evaporation_probability = 1; break;}
			case 7: {evaporation_probability = 1; break;}
		...
		if (Math.random()>evaporation_probability) {return;}
		Phase.evaporate();
	}
