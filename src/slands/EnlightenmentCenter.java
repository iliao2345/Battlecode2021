...
public class EnlightenmentCenter {
	...
	public static void act() throws GameActionException {
		...
		if (ECInfo.guard_flag) {  // guard slanderers
			int test_conviction = Math.max(20, Info.conviction/100);
			if (test_conviction < ECInfo.max_safe_build_limit) {
				if (ECInfo.unhandled_muckraker_loc!=null) {
					...
					if (rc.canBuildRobot(RobotType.POLITICIAN, build_direction2, test_conviction)) {
						Action.buildGuard(build_direction2, test_conviction, ECInfo.unhandled_muckraker_loc, ECInfo.unhandled_muckraker_id_128); return;
					}
				}
			}
		}
		int test_conviction = Math.max(Math.max(50, ECInfo.weakest_ec_influence)+GameConstants.EMPOWER_TAX+1, Info.conviction/2);
