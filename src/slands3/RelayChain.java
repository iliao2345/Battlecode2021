...
public class RelayChain {
	...
	public static int pressure = 7;  // path distance measured as passability weighted euclidean distance
	...
	public static MapLocation signal_loc;
	public static int signal_id_127;
	public static int signal_size;
    ...
    public static void update() throws GameActionException {
    	...
    	pressure = Math.min(pressure+((Info.crowdedness>0.9)?3:1), 7);
		...
		force_dy = 0;
		for (int i=Info.n_relayers; --i>=0;) {
			if (Clock.getBytecodesLeft()>1700 && (rc.getFlag(Info.relayers[i].ID)%8>=pressure) || Info.exterminate) {
		...
		signal_loc = (Info.n_enemy_muckrakers>0)?Info.enemy_muckrakers[0].location:Info.loc;
		signal_id_127 = (Info.n_enemy_muckrakers>0)?Info.enemy_muckrakers[0].ID%127:127;
		if (Info.n_enemy_muckrakers>0) {
			if (Info.enemy_muckrakers[0].conviction<=5) {signal_size = 0;}
			else if (Info.enemy_muckrakers[0].conviction<=10) {signal_size = 1;}
			else if (Info.enemy_muckrakers[0].conviction<=20) {signal_size = 2;}
			else if (Info.enemy_muckrakers[0].conviction<=40) {signal_size = 3;}
			else if (Info.enemy_muckrakers[0].conviction<=100) {signal_size = 4;}
			else if (Info.enemy_muckrakers[0].conviction<=200) {signal_size = 5;}
			else if (Info.enemy_muckrakers[0].conviction<=500) {signal_size = 6;}
			else {signal_size = 7;}
		}
    }
