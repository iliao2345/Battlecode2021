...
//		explore_flag = Info.round_num<450;
		...
		if (!evade) {
			for (int i=Info.n_relayers; --i>=0;) {
				if (Clock.getBytecodesLeft()>1700 && (rc.getFlag(Info.relayers[i].ID)>>6)%128!=127) {
...
//			}
	...
	public static void act() throws GameActionException {
		if (evade) {
		...
		else if (explore_flag || Info.tracked_friendly_ec_ids==null) {
			...
			for (Direction dir:Direction.allDirections()) {
				...
				boolean storage_location = (Info.x+dir.dx+Info.y+dir.dy)%2==1 && !Info.loc.add(dir).isWithinDistanceSquared(ec_location, 8);
			...
			Direction momentum_direction = Info.last_move_direction;  // if cannot avoid odd tiles and friendly EC, try to go out
			...
			for (Direction dir:Math2.UNIT_DIRECTIONS) {
				...
				boolean outgoing_line = (Info.x+dir.dx+Info.y+dir.dy)%2==0 && !Info.loc.add(dir).isWithinDistanceSquared(ec_location, 8);
				if (outgoing_line && (rc.canMove(dir)||dir==Direction.CENTER) && (best_direction==null || cost<lowest_cost)) {
					best_direction = dir;
					lowest_cost = cost;
				}
			}
