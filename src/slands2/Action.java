...
public class Action {
	...
	public static void buildGuard(Direction dir, int conviction, MapLocation target_loc, int id_128) throws GameActionException {
		rc.buildRobot(RobotType.POLITICIAN, dir, conviction);
		acted = true;
		ECInfo.last_build_direction = dir;
		ECInfo.targetting_guard_ids[id_128] = rc.senseRobotAtLocation(Info.loc.add(dir)).ID;
		ECInfo.targetting_guard_signal_rounds[id_128] = Info.round_num;
		ECInfo.guard_ids = new IntCycler(rc.senseRobotAtLocation(Info.loc.add(dir)).ID, ECInfo.guard_ids);
		ECInfo.guard_targets_128 = new IntCycler(id_128, ECInfo.guard_targets_128);
		ECInfo.n_guard_ids++;
		ECInfo.info_transfer_timer = 2;
		ECInfo.targetted_loc = target_loc;
		ECInfo.build_role = 1;
