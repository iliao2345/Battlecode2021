					...
					if (robot==null) {
						...
						for (Direction dir2:Math2.UNIT_DIRECTIONS) {
							...
							if (!target_ec.location.isAdjacentTo(check_loc2) && rc.canSenseLocation(check_loc2)) {
								...
								else if (robot2.team==Info.enemy) {
									for (Direction dir3:Math2.UNIT_DIRECTIONS) {
										MapLocation check_loc3 = check_loc.add(dir3);
										touching_buried_target = touching_buried_target || Info.loc.equals(check_loc3);
										if (!target_ec.location.isWithinDistanceSquared(check_loc3, 8) && rc.canSenseLocation(check_loc3)) {
											RobotInfo robot3 = rc.senseRobotAtLocation(check_loc3);
											if (robot3==null || robot2.team==Info.enemy) {
												need_support = true;
												break loop1;
											}
										}
