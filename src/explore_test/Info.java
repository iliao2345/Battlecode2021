...
public class Info {
	...
	public static int approx_enemy_dist = 0;  // starts from 127 and decreases to 0
	...
	public static void update() throws GameActionException {
		...
		if (!min_x_known) {
			if (x-(min_x-1)<=detect_r) {
				if (rc.canDetectLocation(loc.translate((min_x-1)-x, 0))) {
					min_x -= 1;
				}
				else {
					min_x_known = true;
		...
		if (!max_x_known) {
			if ((max_x+1)-x<=detect_r) {
				if (rc.canDetectLocation(loc.translate((max_x+1)-x, 0))) {
					max_x += 1;
				}
				else {
					max_x_known = true;
		...
		if (!min_y_known) {
			if (y-(min_y-1)<=detect_r) {
				if (rc.canDetectLocation(loc.translate(0, (min_y-1)-y))) {
					min_y -= 1;
				}
				else {
					min_y_known = true;
		...
		if (!max_y_known) {
			if ((max_y+1)-y<=detect_r) {
				if (rc.canDetectLocation(loc.translate(0, (max_y+1)-y))) {
					max_y += 1;
				}
				else {
					max_y_known = true;
		...
		if (sensable_robots.length>30) {sensable_robots = rc.senseNearbyRobots(10);}
		...
		for (RobotInfo robot:friendly_slanderers) {
			int flag = rc.getFlag(robot.getID());
			int robot_enemy_dist = (flag>>Flag.LOG_MAX_APPROX_DIST_PLUS_ONE)%Flag.MAX_APPROX_DIST_PLUS_ONE;
			int robot_home_dist = (flag>>1)%Flag.MAX_APPROX_DIST_PLUS_ONE;
			int additional_cost = (int) (tile_cost*(Math2.length(loc, robot.location)+1));
